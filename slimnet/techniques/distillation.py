...
class IntermediateProjection(nn.Module):
    """Learned projection to align student → teacher hidden dim (TinyBERT)."""
...
class KnowledgeDistillationModule:
    ...
    Usage:
        ...
        trained_student, info = kd.train(teacher, student, train_loader, config, ...)
    """
    ...
    def train(
        ...
        """
        Run distillation. Returns (trained_student, info_dict).
        Teacher is NEVER modified.
        """
        if device is None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # PRD §5.5.3: teacher frozen in eval(), gradients disabled
        ...
        for epoch in range(config.epochs):
            ...
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    loss = self._loss(s_logits, t_logits, labels, config, t_hooks, s_hooks, projs)
            ...
            logger.info(f"[Distillation] epoch {epoch+1}/{config.epochs}  loss={avg_loss:.4f}")

            # Early stopping
            if val_loader is not None and original_accuracy is not None:
                val_acc = self._evaluate(student, val_loader, device)
                logger.info(f"[Distillation] val_acc={val_acc:.4f} (orig={original_accuracy:.4f})")
                ...
                if (original_accuracy - val_acc) > max_accuracy_drop:
                    logger.warning(f"[Distillation] early stop: drop {original_accuracy-val_acc:.4f} > {max_accuracy_drop}")
                    break
        ...
        student.eval()
        info = {"loss_history": history, "final_val_acc": best_val}
        return student, info
    ...
    def _loss(self, s_logits, t_logits, labels, config, t_hooks, s_hooks, projs):
        ...
        # Intermediate feature distillation (TinyBERT)
        if config.intermediate and t_hooks and s_hooks:
            feat_loss = self._feature_loss(t_hooks, s_hooks, projs)
            total = total + 0.1 * feat_loss
    ...
    def _setup_hooks(self, teacher, student, device):
        ...
        projs = [IntermediateProjection(sl[si[i]].out_features, tl[ti[i]].out_features).to(device) for i in range(n)]
        return th, sh, projs
